...
class ProviderEndpoints(private val providerRepository: ProviderClient) {

    @GetMapping("/{region}")
    fun forRegion(@PathVariable region: String): List<Provider> {
        // TODO: Map the passed-in region to an actual region

        return providerRepository.fetchAll()
    }
}
